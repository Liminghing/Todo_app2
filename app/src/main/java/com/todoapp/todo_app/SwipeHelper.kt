...
class SwipeHelper: ItemTouchHelper.Callback() {  // ItemTouchHelper.Callback 을 구현해야 한다
    ...
    override fun getMovementFlags(  // 이동 방향을 결정!!,  스와이프 시 항상 onChildDraw 보다 먼저 호출!
        ...
        var ratio=(view.height.toFloat())/(view.width.toFloat())
        clamp = view.width.toFloat() / 10 * 3.2f   // 아이템뷰 가로 길이의 비율로 clamp 설정도 가능!!!!
    ...
    override fun onChildDraw(  // 스와이프 동작 시 호출
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            ...
            val isClamped = getClamped(viewHolder as MyAdapter.BoardViewHolder)
    ...
    private fun clampViewPositionHorizontal(
        ...
        val maxSwipe: Float = -view.width.toFloat() / 10 * 3.2f // 음수 값!!,  xml 상에서 삭제 영역이 아이템뷰 width의 0.3 만큼 차지하도록 설정한 것과 맞추기 위함
    ...
    override fun getSwipeThreshold(viewHolder: RecyclerView.ViewHolder): Float {   // 터치 유지하다가 떼면 호출됨
        ...
        setClamped(viewHolder as MyAdapter.BoardViewHolder, currentDx <= -clamp)

        return 2f
    }

    private fun getView(viewHolder: RecyclerView.ViewHolder): View {
        return (viewHolder as MyAdapter.BoardViewHolder).itemView.findViewById(R.id.swipe_view) // 아이템뷰에서 스와이프 영역에 해당하는 뷰 가져오기
    }


    private fun setClamped(viewHolder: MyAdapter.BoardViewHolder, isClamped: Boolean){
        viewHolder.itemView.tag = isClamped

    }

    private fun getClamped(viewHolder: MyAdapter.BoardViewHolder): Boolean{
